define([
    ...
    var Pattern = function PatternClosure() {
        function Pattern() {
            a.unreachable('should not call Pattern constructor');
        }
        Pattern.prototype = {
            getPattern: function Pattern_getPattern(ctx) {
                a.unreachable(`Should not call Pattern.getStyle: ${ ctx }`);
            }
        };
        Pattern.parseShading = function (shading, matrix, xref, res, handler, pdfFunctionFactory, localColorSpaceCache) {
            var dict = c.isStream(shading) ? shading.dict : shading;
            ...
            try {
                ...
                default:
                    throw new a.FormatError('Unsupported ShadingType: ' + type);
                }
            } catch (ex) {
                if (ex instanceof d.MissingDataException) {
                    throw ex;
                }
                handler.send('UnsupportedFeature', { featureId: a.UNSUPPORTED_FEATURES.shadingPattern });
                a.warn(ex);
    ...
    var Shadings = {};
    Shadings.SMALL_NUMBER = 0.000001;
    Shadings.RadialAxial = function RadialAxialClosure() {
        function RadialAxial(dict, matrix, xref, resources, pdfFunctionFactory, localColorSpaceCache) {
            ...
            if (Array.isArray(bbox) && bbox.length === 4) {
                this.bbox = a.Util.normalizeRect(bbox);
            } else {
                this.bbox = null;
            }
            var t0 = 0, t1 = 1;
            ...
            if (this.shadingType === ShadingType.RADIAL && (!extendStart || !extendEnd)) {
                ...
                if (r1 <= r2 + distance && r2 <= r1 + distance) {
                    a.warn('Unsupported radial gradient.');
                }
            }
            ...
            for (let i = 0; i <= NUMBER_OF_SAMPLES; i++) {
                ...
                var cssColor = a.Util.makeHexColor(rgbColor[0], rgbColor[1], rgbColor[2]);
                colorStops.push([
                    i / NUMBER_OF_SAMPLES,
                    cssColor
                ]);
            ...
            if (dict.has('Background')) {
                ...
                background = a.Util.makeHexColor(rgbColor[0], rgbColor[1], rgbColor[2]);
            }
            if (!extendStart) {
                colorStops.unshift([
                    0,
        ...
        RadialAxial.prototype = {
            getIR: function RadialAxial_getIR() {
                ...
                if (shadingType === ShadingType.AXIAL) {
                    ...
                    a.unreachable(`getPattern type unknown: ${ shadingType }`);
                }
                var matrix = this.matrix;
                if (matrix) {
                    p0 = a.Util.applyTransform(p0, matrix);
                    p1 = a.Util.applyTransform(p1, matrix);
                    if (shadingType === ShadingType.RADIAL) {
                        var scale = a.Util.singularValueDecompose2dScale(matrix);
                        r0 *= scale[0];
                        r1 *= scale[1];
                    }
                }
    ...
    Shadings.Mesh = function MeshClosure() {
        ...
        function decodeType4Shading(mesh, reader) {
            ...
            while (reader.hasData) {
                ...
                if (verticesLeft === 0) {
                    if (!(0 <= f && f <= 2)) {
                        throw new a.FormatError('Unknown type4 flag');
                    }
                    switch (f) {
                    case 0:
                        verticesLeft = 3;
        ...
        function buildFigureFromPatch(mesh, index) {
            ...
            a.assert(figure.type === 'patch', 'Unexpected patch mesh figure');
        ...
        function decodeType6Shading(mesh, reader) {
            ...
            while (reader.hasData) {
                var f = reader.readFlag();
                if (!(0 <= f && f <= 3)) {
                    throw new a.FormatError('Unknown type6 flag');
                }
                var i, ii;
        ...
        function decodeType7Shading(mesh, reader) {
            ...
            while (reader.hasData) {
                var f = reader.readFlag();
                if (!(0 <= f && f <= 3)) {
                    throw new a.FormatError('Unknown type7 flag');
                }
                var i, ii;
                var pi = coords.length;
        ...
        function Mesh(stream, matrix, xref, resources, pdfFunctionFactory, localColorSpaceCache) {
            if (!c.isStream(stream)) {
                throw new a.FormatError('Mesh data is not a stream');
            }
            ...
            case ShadingType.LATTICE_FORM_MESH:
                ...
                if (verticesPerRow < 2) {
                    throw new a.FormatError('Invalid VerticesPerRow');
                }
            ...
            case ShadingType.TENSOR_PATCH_MESH:
                ...
                break;
            default:
                a.unreachable('Unsupported mesh type.');
                break;
            }
            if (patchMesh) {
                updateBounds(this);
    ...
    function getTilingPatternIR(operatorList, dict, color) {
        ...
        const bbox = a.Util.normalizeRect(dict.getArray('BBox'));
        const xstep = dict.get('XStep');
        const ystep = dict.get('YStep');
        const paintType = dict.get('PaintType');
        const tilingType = dict.get('TilingType');
        if (bbox[2] - bbox[0] === 0 || bbox[3] - bbox[1] === 0) {
            throw new a.FormatError(`Invalid getTilingPatternIR /BBox array: [${ bbox }].`);
        }
        return [
            'TilingPattern',
            color,
