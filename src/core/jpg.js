define([
    ...
    var JpegImage = function JpegImageClosure() {
        ...
        function decodeScan(data, offset, frame, components, resetInterval, spectralStart, spectralEnd, successivePrev, successive, parseDNLMarker = false) {
                ...
                if (bitsData === 255) {
                    ...
                    if (nextByte) {
                        if (nextByte === 220 && parseDNLMarker) {
                            offset += 2;
                            const scanLines = b.readUint16(data, offset);
                            offset += 2;
            ...
            while (mcu <= mcuExpected) {
                ...
                if (fileMarker.invalid) {
                    ...
                    a.warn(`decodeScan - ${ partialMsg } MCU data, current marker is: ${ fileMarker.invalid }`);
        ...
        function findNextFileMarker(data, currentPos, startPos = currentPos) {
            ...
            var currentMarker = b.readUint16(data, currentPos);
            ...
            var newMarker = b.readUint16(data, newPos);
            while (!(newMarker >= 65472 && newMarker <= 65534)) {
                if (++newPos >= maxPos) {
                    return null;
                }
                newMarker = b.readUint16(data, newPos);
            }
            return {
        ...
        JpegImage.prototype = {
            parse(data, {
                ...
                function readDataBlock() {
                    const length = b.readUint16(data, offset);
                    offset += 2;
                    let endOffset = offset + length - 2;
                    var fileMarker = findNextFileMarker(data, endOffset, offset);
                    if (fileMarker && fileMarker.invalid) {
                        a.warn('readDataBlock - incorrect length, current marker is: ' + fileMarker.invalid);
                ...
                let fileMarker = b.readUint16(data, offset);
                offset += 2;
                if (fileMarker !== 65496) {
                    throw new JpegError('SOI not found');
                }
                fileMarker = b.readUint16(data, offset);
                offset += 2;
                markerLoop:
                    while (fileMarker !== 65497) {
                        ...
                        case 65534:
                            ...
                            break;
                        case 65499:
                            const quantizationTablesLength = b.readUint16(data, offset);
                            offset += 2;
                            ...
                            while (offset < quantizationTablesEnd) {
                                ...
                                if (quantizationTableSpec >> 4 === 0) {
                                    ...
                                    for (j = 0; j < 64; j++) {
                                        z = dctZigZag[j];
                                        tableData[z] = b.readUint16(data, offset);
                                        offset += 2;
                                    }
                                } else {
                        ...
                        case 65474:
                            ...
                            const sofScanLines = b.readUint16(data, offset);
                            offset += 2;
                            frame.scanLines = dnlScanLines || sofScanLines;
                            frame.samplesPerLine = b.readUint16(data, offset);
                            offset += 2;
                            frame.components = [];
                            frame.componentIds = {};
                            ...
                            break;
                        case 65476:
                            const huffmanLength = b.readUint16(data, offset);
                            offset += 2;
                        ...
                        case 65501:
                            offset += 2;
                            resetInterval = b.readUint16(data, offset);
                            offset += 2;
                            break;
                        case 65498:
                            ...
                            try {
                                ...
                                if (ex instanceof DNLMarkerError) {
                                    a.warn(`${ ex.message } -- attempting to re-parse the JPEG image.`);
                                    return this.parse(data, { dnlScanLines: ex.scanLines });
                                } else if (ex instanceof EOIMarkerError) {
                                    a.warn(`${ ex.message } -- ignoring the rest of the image data.`);
                                    break markerLoop;
                                }
                                throw ex;
                            }
                        ...
                        default:
                            ...
                            if (nextFileMarker && nextFileMarker.invalid) {
                                a.warn('JpegImage.parse - unexpected data, current marker is: ' + nextFileMarker.invalid);
                                offset = nextFileMarker.offset;
                                break;
                            }
                            if (!nextFileMarker || offset >= data.length - 1) {
                                a.warn('JpegImage.parse - reached the end of the image data ' + 'without finding an EOI marker (0xFFD9).');
                                break markerLoop;
                            }
            ...
            getData({width, height, forceRGB = false, isSourcePDF = false}) {
                if (typeof PDFJSDev === 'undefined' || PDFJSDev.test('!PRODUCTION || TESTING')) {
                    a.assert(isSourcePDF === true, 'JpegImage.getData: Unexpected "isSourcePDF" value for PDF files.');
                }
