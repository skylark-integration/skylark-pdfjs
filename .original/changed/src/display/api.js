...
class PDFPageProxy {
   ...
   */
  getJSActions() {
    return (this._jsActionsPromise = this._jsActionsPromise || this._transport.getPageJSActions( // lwf
      this._pageIndex
    ));
  }
  ...
  _abortOperatorList({ intentState, reason, force = false }) {
    ...
    intentState.streamReader.cancel(new AbortException(reason && reason.message)); //lwf
    intentState.streamReader = null;
...
const PDFWorker = (function PDFWorkerClosure() {
  ...
  function setupFakeWorkerGlobal() {
    ...
    const loader = async function () {
      ...
      if (typeof PDFJSDev === "undefined" || !PDFJSDev.test("PRODUCTION")) { 
        //// const worker = await import("pdfjs/core/worker.js"); // lwf
  ...
  function createCDNWrapper(url) {
    ...
    ///const wrapper = "importScripts('" + url + "');"; // lwf
...
class WorkerTransport {
  ...
  hasJSActions() {
    return (this._hasJSActionsPromise =  this._hasJSActionsPromise || this.messageHandler.sendWithPromise( //lwf
      "HasJSActions",
      null
    ));
  }
